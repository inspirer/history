...
int ReadFile(){
  if (FileOpened>0){
    ...
    if (tfs->e==tfs->i){
     tfs->i=fread(tfs->swp,1,fbuff_size,tfs->fp);
...
int OpenFile(char *fname){
  ...
  FileOpened++;tfs->lines=1;
  tfs->FileName=(char *)malloc(strlen(fname)+1);
  if (tfs->FileName==NULL) return 1;strcpy(tfs->FileName,fname);
